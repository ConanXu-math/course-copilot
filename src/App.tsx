...
import { getConversation, listConversations, saveBookMetadata, savePageText, uploadBook } from './lib/storage';
...
export default function App() {
  ...
  useEffect(() => {
    setOpenTabs([]); setActiveTab('textbook'); setSelectedText(''); setPageText(''); setQuery('');
  ...
  const textReady = useCallback((text: string) => {
    setPageText(text);
    if (book && text) void savePageText(book.id, page, text).catch(error => setToast(error.message));
  }, [book?.id, page]);
  ...
  function goToPage(next:number) {
    ...
    setPage(Math.max(1,Math.min(next,book?.totalPages || next))); setSelectedText(''); setActiveTab('textbook'); setMobileView('reader');
    if(window.innerWidth<900) setOutlineOpen(false);
  }
  function openArtifact(artifact:Artifact) { setOpenTabs(current=>current.includes(artifact.id)?current:[...current,artifact.id]); setActiveTab(artifact.id); setMobileView('reader'); }
  function closeTab(id:string) { setOpenTabs(current=>current.filter(tab=>tab!==id)); if(activeTab===id) setActiveTab('materials'); }
  ...
  return <div className="app-shell">
    ...
    <div ref={columns.workspaceRef} style={columns.style} inert={workspace.switching || moving} aria-busy={workspace.switching || moving} className={`workspace ${outlineOpen?'':'outline-closed'} mobile-${mobileView} ${columns.dragging ? 'is-resizing' : ''}`}>
      ...
      <main id="course-reading" className="reading-panel">
        <div className="reading-tabs"><button className="icon-button outline-open-button" aria-label="展开教材目录" onClick={()=>setOutlineOpen(!outlineOpen)}><PanelLeft size={17}/></button><button className={`reading-tab ${activeTab==='textbook'?'active':''}`} onClick={()=>setActiveTab('textbook')}><BookOpen size={16}/>教材</button><button className={`reading-tab ${activeTab==='materials'?'active':''}`} onClick={()=>setActiveTab('materials')}><FolderOpen size={16}/>学习资料<span className="count-badge">{artifacts.length}</span></button>{openTabs.map(id=>{const artifact=artifacts.find(item=>item.id===id);return artifact?<div className={`result-tab ${activeTab===id?'active':''}`} key={id}><button onClick={()=>setActiveTab(id)}>{artifact.title}</button><button aria-label={`关闭${artifact.title}`} onClick={()=>closeTab(id)}><X size={12}/></button></div>:null;})}<button className={`icon-button bookmark-button ${bookmarks.includes(page)?'marked':''}`} disabled={!book} onClick={()=>{setBookmarks(current=>current.includes(page)?current.filter(number=>number!==page):[...current,page]);setToast(bookmarks.includes(page)?'已移除书签。':`已收藏第 ${page} 页。`);}} aria-label={bookmarks.includes(page)?'移除本页书签':'收藏本页'} title={bookmarks.includes(page)?'移除本页书签':'收藏本页'}><Bookmark size={17} fill={bookmarks.includes(page)?'currentColor':'none'}/></button></div>
        {book ? <>{activeTab==='textbook' && <div className="reading-location"><span>{mainChapter ? `第 ${topChapters.findIndex(item=>item.id===mainChapter.id)+1} 章`:'教材'}</span><ChevronRight size={12}/><strong>{chapter?.title || book.title}</strong><span className="original-label">教材原文</span></div>}<div className={`reader-mount ${activeTab==='textbook'?'':'hidden'}`}><TextbookReader book={book} page={page} onPageChange={goToPage} onDocumentReady={documentReady} onTextChange={textReady} onSelectionChange={setSelectedText}/></div>{activeTab==='materials' && <MaterialsLibrary artifacts={artifacts} onOpen={openArtifact}/>} {activeArtifact && <ArtifactViewer key={activeArtifact.id} artifact={activeArtifact} onPage={goToPage} book={book}/>}</> : <div className="boot-state">{bootError ? <><BookOpen size={36}/><h2>先打开一本教材</h2><p>{bootError}</p><button className="primary-button" onClick={()=>fileInput.current?.click()}><Upload size={16}/>导入 PDF</button></> : <><LoaderCircle className="spin" size={28}/><p>正在准备你的课程空间…</p></>}</div>}
      </main>

      {book && <CopilotPanel book={book} page={page} chapter={chapter} selectedText={selectedText} onClearSelection={clearSelection} skills={skills} messages={messages} busy={busy || workspace.switching || moving} onSend={sendSkill} onStop={stopTask} onReset={()=>void newConversation()} onHistory={()=>void openHistory()} onArtifact={openArtifact} onSettings={()=>setShowSettings(true)}/>}
    </div>
