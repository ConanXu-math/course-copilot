...
async function runAgent(req, res, request, skills) {
  ...
  try {
    ...
    try { await saveGeneratedArtifact.finalize(); }
    catch (error) {
      console.error('恢复课程资料失败：', error);
      if (!controller.signal.aborted && !res.destroyed) res.write(`${JSON.stringify({ type: 'error', message: '课程资料收尾失败，请查看服务端输出。' })}\n`);
    } finally {
