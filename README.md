...
个人部署的课程学习工作台：左侧目录，中间阅读教材或查看生成资料，右侧使用 Copilot 的课程工具、对话和输入框。本分支兼容 `main` 的 `bb9dc4f8` 版本，保留其三栏界面与 ACP 多 Agent 连接，并新增连续 PDF、图谱编辑、分类排序及结果保存保护。PR 以该主分支版本为比较基准，上游能力不重复计入本分支贡献。整个项目按三层分工：
...
**支持个人部署，选择 Agent 后连接即可，默认统一使用 ACP。** 主分支提供多种 Agent 与自定义连接方式，Codex、Claude 的 ACP 适配器随项目安装；原生接口和非交互命令行可在高级设置中选择。每个人使用自己的 Agent 账号。本分支包含教材解析、讲解内容、知识点出题、思维导图、知识图谱和 LaTeX Beamer 课件 Skill，讲解视频保留自定义 Skill 接入位置。各项任务通过所选 Agent 执行；路径可读不代表任务已执行成功。
