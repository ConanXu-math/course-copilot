...
description: 从真实教材提炼概念及有正文依据的有向关系，生成知页课程 Copilot 可展示并回跳 PDF 页码的知识图谱；适用于当前节、当前章、全书或选文的概念联系分析。
---

# 教材知识图谱

把选定教材内容整理为“概念—关系→概念”的网络，保存为知页的 `knowledge-graph` 资料。重点呈现概念之间的作用、依赖、应用、条件与区别，默认用中文。新图谱使用 v2 格式；生成前读取 [数据约定](references/schema.md)，字段定义和完整示例以该文件为准。
...
- 兼容旧请求的 `page`：以当前 PDF 页正文为范围，必要时读相邻页理解上下文。

任务给出的解析目录通常有 `outline.json`（`level=0` 为章、`level=1` 为节）和 `pages/` 缓存。缓存只包含已经阅读的页面。目录页码可能有偏移，起止页也可能跨节；读取起始页和下一个同级/更高层级标题所在页，按正文标题截取，不把页眉当边界，也不机械减去一页。缺少可靠目录时依据正文定位，无法确定则说明缺口，不自行扩大范围。

使用本 Skill 的 [read-pages.mjs](scripts/read-pages.mjs) 补读真实 PDF；它复用同仓库 `skills/mindmap/scripts/read-pages.mjs`，依赖项目已有 Node.js 与 PDF.js。两个 Skill 随仓库一起交付，不依赖当前工作目录，也不需要另装读取工具。

```sh
node "/本Skill绝对路径/scripts/read-pages.mjs" "/本次教材路径.pdf" 9 17
```
