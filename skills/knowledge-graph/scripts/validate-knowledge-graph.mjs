...
const isPositivePage = value => Number.isSafeInteger(value) && value >= 1;
const isPage = (value, totalPages) => isPositivePage(value) && value <= totalPages;

function validateCoverage(result, pageCheck, pageRangeLabel) {
  ...
  for (const [index, item] of coverage.items.entries()) {
    ...
    if (!Array.isArray(item.pages) || item.pages.some(page => !pageCheck(page))) {
      throw new Error(`${field}.pages 必须是由${pageRangeLabel}的整数构成的数组。`);
    }
...
function validateV2Edge(edge, edgeIds, pageCheck, pageRangeLabel) {
  ...
  for (const [index, entry] of edge.evidence.entries()) {
    ...
    if (!isObject(entry) || !pageCheck(entry.page)) {
      throw new Error(`${field}.page 必须是${pageRangeLabel}的整数。`);
    }
...
export function validateKnowledgeGraph(result, expectedId, totalPages, options = {}) {
  const relaxPageBounds = options.historicalRead === true;
  const pageCheck = relaxPageBounds
    ? isPositivePage
    : (value) => isPage(value, totalPages);
  const pageRangeLabel = relaxPageBounds ? '大于等于 1' : `在 1–${totalPages} 范围内`;
  if (!isText(expectedId)) throw new Error('预期结果 ID 不能为空。');
  if (!relaxPageBounds && (!Number.isSafeInteger(totalPages) || totalPages < 1)) {
  ...
  if (isV2) validateCoverage(result, pageCheck, pageRangeLabel);
  ...
  for (const node of result.nodes) {
    ...
    if (node.page !== undefined && !pageCheck(node.page)) {
      throw new Error(`节点 ${node.id} 的 page 必须是${pageRangeLabel}的整数。`);
    }
  ...
  for (const edge of result.edges) {
    ...
    if (isV2) validateV2Edge(edge, edgeIds, pageCheck, pageRangeLabel);
